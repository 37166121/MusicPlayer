    ...
    private void flattenPreferenceGroup(List<Preference> preferences, PreferenceGroup group) {
        ...
        for (int i = 0; i < preferences.size(); i++) {
            ...
            if (preference instanceof PreferenceCategory) {
                if (i > 0) {
                    preferences.get(i - 1).setPreferenceType(Preference.PreferenceType.FOOTER);
                }
                preference.setPreferenceType(Preference.PreferenceType.HEADER);
            } else {
                if (i == preferences.size() - 1) {
                    preference.setPreferenceType(Preference.PreferenceType.FOOTER);
                }
            }
        }
    }
